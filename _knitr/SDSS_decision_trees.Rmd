...
The following three code chunks extract and clean the training data for all three sources and combine them in to a single data frame. Quasras, stars and white dwarfs are given the labels 1,2 and 3 respectively. There are 5000 stellar objects available for training, but for quasars there are over `r round(nrow(read.table('http://astrostatistics.psu.edu/MSMA/datasets/SDSS_QSO.dat', h=T)),2)` and for white dwarfs over `r round(nrow(read.csv('http://astrostatistics.psu.edu/MSMA/datasets/SDSS_wd.csv', h=T)),2)`, so I've filtered each of the latter two down to only 5000 so that there are equal numbers of each class.
...
The overall test error rate is `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%.
...
```{r echo=FALSE}
min_size = cv.sdss$size[max(which(cv.sdss$dev == min(cv.sdss$dev)))]
max_size = cv.sdss$size[min(which(cv.sdss$dev == min(cv.sdss$dev)))]
```

The `r min_size` branch tree has the same error rate as the `r max_size`, so pruning to this size will not reduce the predictive power of the model, but will reduce the complexity.
...
The error rate, `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%, is the same, as expected, but the tree is easier to interpret.
...
The test error rate associated with the bagged tree is `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%, a significant improvement over the single decision tree.
...
The test error rate associated with the random forest is `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%, a further improvement over the bagged tree.
...
Test error rate associated with Boosting is `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%. This is marginally better than the bagging and random forest approaches above, but performance is $~\mathcal{O}(10)$ worse.
...
Test error rate associated with ERT `r round(100*(1-sum(diag(test.results))/sum(test.results)),2)`%.
...
plot(SDSS_test[,1], SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8),pch=20,
	col=SDSS_test[,5], cex=0.6, cex.lab=1.5, cex.axis=1.5, main='', xlab='u-g (mag)', ylab='g-r (mag)')
legend(-0.5, 1.7, c('QSO','MS + RG','WD'), pch=20, col=c('black','red','green'),
	cex=)
plot(SDSS_test[,2], SDSS_test[,3], xlim=c(-0.7,1.8), ylim=c(-0.7,1.8), pch=20,
	col=SDSS_test[,5], cex=0.6, cex.lab=1.5, cex.axis=1.5, main='', xlab='g-r (mag)', ylab='r-i (mag)')
plot(SDSS_test[,3], SDSS_test[,4], xlim=c(-0.7,1.8), ylim=c(-1.1,1.3), pch=20,
	col=SDSS_test[,5], cex=0.6, cex.lab=1.5, cex.axis=1.5, main='', xlab='r-i (mag)', ylab='i-z (mag)')
title("test data", outer=TRUE)
```
...
plot(SDSS_train[,3], SDSS_train[,4], xlim=c(-0.7,1.8), ylim=c(-1.1,1.3), pch=20,
	...
	ylab='i-z (mag)')
par(mfrow=c(1,1))
title("training data", outer=TRUE)
```
